...
import { perinatalCourses } from '../educationData';

function PerinatalPanel({ isActive }) {
  return (
    <div className={`edu-tab-panel${isActive ? ' active' : ''}`} role="tabpanel" aria-label="Perinatal Education">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px,1fr))', gap: '1rem', marginBottom: '2rem' }}>
        {perinatalCourses.map(({ title, desc, tag }) => (
          <div key={title} style={{
